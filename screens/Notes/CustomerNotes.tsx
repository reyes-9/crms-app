...
import { CustomerNoteDetails } from '@/types/customerNote';
...
export const CustomerNotes = () => {
  const { customerNotes, isLoading, getCustomerNotes, deleteCustomerNote } =
    useCustomerNote();
  ...
  const [editingNote, setEditingNote] = useState<CustomerNoteDetails | null>(
    null,
  );
  ...
  const filtered = customerNotes.filter(
    (n) =>
  ...
  const handleOpenEdit = (note: CustomerNoteDetails) => {
    setOpenMenu(null);
    setEditingNote(note);
    setModalVisible(true);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await deleteCustomerNote(deleteTarget);
    } catch {
...
interface NoteFormModalProps {
  visible: boolean;
  onClose: () => void;
  editingNote: CustomerNoteDetails | null;
  customer_id: number;
}
...
const NoteFormModal = ({
  ...
  const { addCustomerNote, editCustomerNote } = useCustomerNote();
