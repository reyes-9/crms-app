...
const SORT_OPTIONS: SortOption[] = ['all', 'new', 'contacted', 'qualified', 'unqualified', 'converted', 'lost'];

const DUMMY_LEADS: Lead[] = [
  { id: '1', name: 'John Doe',          company: 'Acme Corp',         email: 'john@acme.com',      number: '09171234567', status: 'new',       source: 'website' },
  { id: '2', name: 'Jane Smith',         company: 'Globex Inc',        email: 'jane@globex.com',    number: '09181234567', status: 'contacted',  source: 'referral' },
  { id: '3', name: 'Michael Johnson',    company: 'Initech',           email: 'michael@initech.com',number: '09192345678', status: 'qualified',  source: 'email' },
  { id: '4', name: 'Emily Davis',        company: 'Umbrella Co',       email: 'emily@umbrella.com', number: '09175551234', status: 'lost',       source: 'social' },
  { id: '5', name: 'Robert Brown',       company: 'Soylent Corp',      email: 'robert@soylent.com', number: '09176667777', status: 'new',        source: 'website' },
  { id: '6', name: 'Laura Wilson',       company: 'Stark Industries',  email: 'laura@stark.com',    number: '09178889999', status: 'contacted',  source: 'referral' },
  { id: '7', name: 'David Miller',       company: 'Wayne Enterprises', email: 'david@wayne.com',    number: '09179990000', status: 'qualified',  source: 'website' },
  { id: '8', name: 'Sophia Taylor',      company: 'Wonka Industries',  email: 'sophia@wonka.com',   number: '09171112222', status: 'lost',       source: 'social' },
  { id: '9', name: 'James Anderson',     company: 'Cyberdyne Systems', email: 'james@cyberdyne.com',number: '09173334444', status: 'new',        source: 'email' },
  { id: '10',name: 'Olivia Martinez',    company: 'Tyrell Corp',       email: 'olivia@tyrell.com',  number: '09174445555', status: 'contacted',  source: 'website' },
];
...
export const LeadScreen = () => {
  ...
  const filtered = selectedSort === 'all'
    ? DUMMY_LEADS
    : DUMMY_LEADS.filter((l) => l.status === selectedSort);

  const onRefresh = async () => {
    setRefreshing(true);
    try { /* await getLeads() */ } finally { setRefreshing(false); }
  };
    ...
    <>
      ...
      <View style={styles.sortWrapper}>
        ...
        >
          <View style={[
            styles.sortDot,
            selectedSort !== 'all' && { backgroundColor: STATUS_COLOR[selectedSort as LeadStatus]?.color ?? DS.color.textPrimary },
            selectedSort === 'all' && { backgroundColor: DS.color.textPrimary },
          ]} />
          <Text style={styles.sortTriggerText}>
            {selectedSort === 'all' ? 'All Leads' : STATUS_LABEL[selectedSort as LeadStatus]}
          </Text>
          <Feather name={sortMenuOpen ? 'chevron-up' : 'chevron-down'} size={16} color={DS.color.textSecondary} />
        </Pressable>
            ...
            {SORT_OPTIONS.map((opt) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => { setSelectedSort(opt); setSortMenuOpen(false); }}
                >
                  {opt !== 'all' && (
                    <View style={[styles.sortDot, { backgroundColor: STATUS_COLOR[opt as LeadStatus]?.color }]} />
                  )}
                  <Text style={[styles.sortOptionText, isActive && styles.sortOptionTextActive]}>
                    {opt === 'all' ? 'All Leads' : STATUS_LABEL[opt as LeadStatus]}
                  </Text>
                  {isActive && <Feather name="check" size={14} color={DS.color.primary} style={{ marginLeft: 'auto' }} />}
                </Pressable>
              );
            })}
          </View>
  ...
  return (
    <Pressable style={{ flex: 1, backgroundColor: DS.color.bg }} onPress={() => setSortMenuOpen(false)}>
      <Animated.FlatList
        data={filtered}
...
const LEAD_NOTES = [
  { id: '1', content: 'Followed up with client regarding proposal.',  createdAt: '2025-04-10' },
  { id: '2', content: 'Client requested a demo next week.',            createdAt: '2025-04-12' },
  { id: '3', content: 'Sent updated contract for review.',             createdAt: '2025-04-15' },
  { id: '4', content: 'Scheduled demo for April 20.',                  createdAt: '2025-04-16' },
];

const SAMPLE_LEAD: LeadProfile = {
  id: 'LEAD001', name: 'Alice Johnson', company: 'Tech Solutions Inc.',
  email: 'alice.johnson@example.com', number: '+1234567890',
  status: 'new', source: 'website', notes: 'Interested in product demo.', value: 50000,
};
...
export const LeadDetailsScreen = () => {
  ...
  const initials = SAMPLE_LEAD.name.split(' ').slice(0, 2).map((w) => w[0]).join('').toUpperCase();

  return (
    <ScrollView style={styles2.screen} contentContainerStyle={styles2.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles2.profileCard}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles2.profileMeta}>
            <View style={[styles2.statusPill, { backgroundColor: STATUS_COLOR[currentStatus as LeadStatus]?.bg }]}>
              <Text style={[styles2.statusPillText, { color: STATUS_COLOR[currentStatus as LeadStatus]?.color }]}>
                {STATUS_LABEL[currentStatus as LeadStatus] ?? currentStatus}
              </Text>
            </View>
            <Text style={styles2.valuePill}>{formatCurrency(SAMPLE_LEAD.value, 'en-PH', 'PHP')}</Text>
          </View>
        </View>
      ...
      <View style={styles2.actionsRow}>
        ...
        <TouchableOpacity style={[styles2.actionBtn, styles2.actionBtnPrimary]}>
          ...
          <Text style={[styles2.actionBtnText, { color: DS.color.textInverse }]}>Advance Lead</Text>
        </TouchableOpacity>
      </View>
      ...
      <SectionCard2 title="Pipeline Stage" icon="git-branch">
        <View style={styles2.pipeline}>
          {STATUS_CHOICES.map(([value, label], index) => {
            ...
            const isCurrent = index === currentIndex && !['unqualified', 'lost'].includes(currentStatus);
            const isFailed  =
            ...
            const text = currentStatus === 'unqualified' && index === 2 ? 'Unqualified'
              : currentStatus === 'lost' && index === 3 ? 'Lost' : label;
      ...
      <SectionCard2 title="Lead Info" icon="user">
        {[
          ...
          { label: 'Status',  value: STATUS_LABEL[currentStatus as LeadStatus] ?? currentStatus },
          { label: 'Source',  value: SAMPLE_LEAD.source.charAt(0).toUpperCase() + SAMPLE_LEAD.source.slice(1) },
          { label: 'Value',   value: formatCurrency(SAMPLE_LEAD.value, 'en-PH', 'PHP') },
        ].map(({ label, value }, i) => (
          <View key={label}>
...
const EmptyState = () => (
  <View style={styles.emptyState}>
    ...
    <Text style={styles.emptyDesc}>Try a different filter or add a new lead.</Text>
  </View>
);
...
const styles = StyleSheet.create({
  ...
  pageHeader: {
    ...
    paddingHorizontal: DS.spacing.xl, paddingTop: DS.spacing.xl, paddingBottom: DS.spacing.md,
  },
  ...
  // ADD BUTTON
  addButton: {
    marginHorizontal: DS.spacing.xl, height: 52, borderRadius: DS.radius.md,
    backgroundColor: DS.color.primary,
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 8, ...DS.shadow.sm, marginBottom: DS.spacing.md,
  },
  addButtonText: { fontSize: 15, fontWeight: '600', color: DS.color.textInverse },

  // SORT
  sortWrapper: {
    marginHorizontal: DS.spacing.xl, marginTop: DS.spacing.md, zIndex: 50,
  },
  ...
  sortTrigger: {
    ...
    borderRadius: DS.radius.md, paddingHorizontal: DS.spacing.md, height: 46,
  },
  sortDot: { width: 8, height: 8, borderRadius: DS.radius.full, backgroundColor: DS.color.textPrimary },
  sortTriggerText: { flex: 1, fontSize: 13, fontWeight: '600', color: DS.color.textPrimary },
  sortDropdown: {
    marginTop: 6, backgroundColor: DS.color.card,
    borderWidth: 1, borderColor: DS.color.border,
    borderRadius: DS.radius.md, overflow: 'hidden', ...DS.shadow.md,
  },
  sortOption: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 11, paddingHorizontal: DS.spacing.md },
  ...
  // LIST
  listLabel: { ...DS.typography.eyebrow, paddingHorizontal: DS.spacing.xl, marginTop: DS.spacing.lg, marginBottom: DS.spacing.sm },
  ...
  leadCard: {
    ...
    borderWidth: 1, borderColor: DS.color.border,
    padding: DS.spacing.lg, marginBottom: DS.spacing.sm,
    flexDirection: 'row', alignItems: 'center', gap: DS.spacing.md,
    ...DS.shadow.sm,
  },
  leadAvatar: { width: 44, height: 44, borderRadius: DS.radius.full, alignItems: 'center', justifyContent: 'center' },
  ...
  leadMetaText: { fontSize: 11, color: DS.color.textMuted, textTransform: 'capitalize' },
  leadStatusBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: DS.radius.full },
...
const styles2 = StyleSheet.create({
  ...
  profileCard: {
    backgroundColor: DS.color.card, borderRadius: DS.radius.lg,
    borderWidth: 1, borderColor: DS.color.border,
    padding: DS.spacing.lg, flexDirection: 'row', alignItems: 'flex-start', gap: DS.spacing.md,
    ...DS.shadow.sm,
  },
  avatar: {
    width: 52, height: 52, borderRadius: DS.radius.full,
    backgroundColor: DS.color.primaryLight, alignItems: 'center', justifyContent: 'center',
  },
  ...
  statusPill: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: DS.radius.full },
  statusPillText: { fontSize: 11, fontWeight: '600' },
  valuePill: { fontSize: 13, fontWeight: '600', color: DS.color.textPrimary },
  editBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 5,
    backgroundColor: DS.color.primaryMuted, paddingHorizontal: 12, paddingVertical: 8,
    borderRadius: DS.radius.md, borderWidth: 1, borderColor: DS.color.primaryLight,
  },
  ...
  actionBtn: {
    ...
    gap: 6, height: 46, backgroundColor: DS.color.card,
    borderRadius: DS.radius.md, borderWidth: 1, borderColor: DS.color.border,
  },
  actionBtnPrimary: { backgroundColor: DS.color.primary, borderColor: DS.color.primary, flex: 1.5 },
  actionBtnText: { fontSize: 13, fontWeight: '600', color: DS.color.primary },

  // PIPELINE
  pipeline: { flexDirection: 'row', borderRadius: DS.radius.sm, overflow: 'hidden', borderWidth: 1, borderColor: DS.color.border },
  pipelineStage: { flex: 1, paddingVertical: 8, alignItems: 'center', justifyContent: 'center' },
  ...
  sectionCard: {
    backgroundColor: DS.color.card, borderRadius: DS.radius.lg,
    borderWidth: 1, borderColor: DS.color.border, overflow: 'hidden', ...DS.shadow.sm,
  },
  sectionHeader: {
  ...
  sectionIconWrap: {
    width: 28, height: 28, borderRadius: DS.radius.xs,
    backgroundColor: DS.color.primaryMuted, alignItems: 'center', justifyContent: 'center',
  },
  sectionTitle: { fontSize: 15, fontWeight: '700', color: DS.color.textPrimary },
  sectionAction: { flexDirection: 'row', alignItems: 'center', gap: 2 },
  sectionActionText: { fontSize: 13, fontWeight: '600', color: DS.color.primary },
  sectionBody: { padding: DS.spacing.lg },

  rowDivider: { height: 1, backgroundColor: DS.color.borderLight },
  infoRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: DS.spacing.sm },
  infoLabel: { fontSize: 13, color: DS.color.textSecondary },
  infoValue: { fontSize: 13, fontWeight: '500', color: DS.color.textPrimary, textAlign: 'right', maxWidth: '60%' },
